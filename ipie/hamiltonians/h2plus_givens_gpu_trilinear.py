"""
GPU-friendly 3-angle Givens-grid master equation with trilinear deposition.
...
then deposited onto the 2^3 neighboring midpoint-grid nodes.  If an
interpolation neighbor is outside [-pi/2,pi/2]^3, it is mapped back into the
principal chart using the exact determinant identification, including the
overall sign.  The boundary map is generated geometrically, so edges and
corners are handled consistently without manually composing face rules.
"""
...
class GivensMasterEquation:
    ...
    def __init__(self, Ng: int = 33, importance: bool = True, nodal_tol: float = 1.0e-13):
    ...
    def mat_vec(
        ...
        """Apply the trilinear transfer operator on CPU or GPU.

        Memory scales as O(term_chunk * source_chunk), not O(nterms * Ng^3).
        Each of the eight stencil corners is accumulated immediately with
        xp.bincount, avoiding an eightfold persistent map.
        """
        if self.Us is None:
        ...
        for a0 in range(0, nterms, term_chunk):
            ...
            for s0 in range(0, self.Nh, source_chunk):
                ...
                # Shape: (nterm_chunk, nsource_chunk, 4).
                ...
                if self.importance:
                    base = base / self.trial_mos[None, s0:s1]

                # Deposit to 2^3 neighboring midpoint nodes.  Boundary sign is
                # the sign in |phi_ghost> = sign * |phi_interior>.  Corners
                # are batched so the GPU performs fewer full-length bincounts.
                dest_batch = []
                vals_batch = []
    ...
    def normalize(self, weight):
        ...
        if float(to_host(nrm)) < self.nodal_tol:
            raise RuntimeError("Reconstructed many-body vector has near-zero norm.")
        return weight / nrm
...
# Example integration with the user's existing driver:
#
#   me = GivensMasterEquation(Ng=65, importance=True)
#   me.build(ham, trial)
...
#   )
#
# On a GPU, tune term_chunk and source_chunk upward until memory usage is near
# but below the available device memory.
