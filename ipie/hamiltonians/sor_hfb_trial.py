    ...
    def calc_trial_ovlp_ratio(self,walkers,trial,compute_R0=True,compute_R=True):
        ...
        for d,terms in enumerate(self.terms):
            ...
            for i,term in enumerate(terms):
                ...
                mk12 = np.einsum('wij,wjk->wik',m,k12)
                tr -= 2.*np.einsum('wij,wij->w',tmp,mk12)

                # (0,1,0,1)
                k = _select(Ky11,(0,1,3),term)
                ky11 = _block(k,ns,sign=1,copy=True)
                right = -np.einsum('wij,wjk->wik',m,ky11)
                right = np.einsum('wij,wjk->wik',right,m) 
                # (0,1,1,1)
                tmp = np.einsum('wij,jk->wik',mk12,c2)
                right += np.einsum('wij,wkj->wik',tmp,mk12) 
                ...
                left += 2.*np.einsum('wij,wjk->wik',k12c,k2p)
                # (1,1,0,1)
                # (1,1,1,1)
                tmp = ky11 + np.einsum('wij,wkj->wik',k12c,k12)
                left += np.einsum('wij,wjk->wik',tmp,mk1p)
