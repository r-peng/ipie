...
class BatchTerms:
    ...
    def add_term(self,ai,pi,gi):
        ...
        # compute spin-seperated info
        ps = [[],[]]
        ds = [[],[]]
        for p,d in zip(to_host(pi),to_host(di)):
        ...
        for s in (0,1):
            if len(ps[s])==0:
                self.spin_info[r,i,s] = None 
            else:
                self.spin_info[r,i,s] = xp.asarray(ps[s]),xp.asarray(ds[s])
    ...
    def apply_rotation(self,C,r,i):
        ...
        for s in (0,1):
            ...
            if self.chol_basis is None:
                ...
                vec = self.chol_basis[:,p]
                C[s] += xp.outer(vec,d*xp.dot(vec.T,C[s]))
        return C
