...
def _UB(psi,chol_basis):
    if psi is None:
        return None
    return xp.einsum('dxp,xi->dpi',chol_basis,psi)

@plum.dispatch
def preprocess_trial_and_hamiltonian(trial:SingleDet,ham):
    trial.psi = [trial.psi0a.real,None]
    if trial.nelec[1]>0:
        trial.psi[1] = trial.psi0b.real
    ham.UB = [_UB(psi,ham.chol_basis) for psi in trial.psi]
    _preprocess_trial(trial)

@plum.dispatch
def preprocess_trial(trial:SingleDetGHF,hamiltonian):
    trial.psi = trial.psi0.real
    ham.UB = [_UB(trial.psi[:trial.nbasis],ham.chol_basis),_UB(trial.psi[trial.nbasis:],ham.chol_basis)]
    _preprocess_trial(trial)
